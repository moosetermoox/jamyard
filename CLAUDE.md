...
## Current Snapshot
- **1751 tests passing** (`npm test`, ~5s) · **321 prompts** across 3 banks (`recipes/prompt-banks/`)
...
- `/prototype` ("the simulator", teacher copy "Try it out") host + player iframes side-by-side for playtesting; the host panel has two tabs, Class screen and Teacher controls (the real `/teacher` console auto-joined via `#code=&pin=`, 2026-09-07); "Add sample answers" deals the template's `sampleAnswers` by seat; the map rail is clickable: a stop row asks "Skip ahead?" then plays the room forward with bots until the live rail reaches that step (`?goto=<phaseId>` deep link does the same on launch)
