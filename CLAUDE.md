...
- `/make?game=<id>` — **Make it yours as a page** (2026-09-09, `screens/make/`): the first student step as the class will see it (`GET /api/games/:id/print`, `engine/make-print.js`), the question and field labels editable in place, a tappable timer on plain configs, one red TRY IT (opens Try it out), Host it now, the designer link, and **the setup card** beside the doors (2026-09-13, `.setup-card`: one line per setting with its value as a chip, yellow = set, dashed = not; Your class, the AI's one or two questions, Student names, Early-bird joke; a tap opens the control in the panel under the doors, the switches flip on the spot); the AI's questions carry `kind: choice|text` + `label` (`AIService.shapeCustomizeQuestion`), choice = a row of chips with "Something else…", text = the plank with the mic, one question the wording hangs on and a second only if the content depends on it; edits apply through `POST /api/games/:id/make`; untouched = the original runs, no copy. `MakeItYours.open` routes here for every template; a recipe's own editor (quiz, bluff, or setup knobs) mounts under the doors via `MakeItYours.mountPanel` (the print is read-only then, the panel's `makeCopy` saves). The Create page lands here too after a recipe match saves its copy (2026-09-12); a copy that is already yours (`MyGames.has`) saves back to itself with PUT, never a second copy. The old dialog stays behind `window.MAKE_IT_YOURS_DIALOG`
