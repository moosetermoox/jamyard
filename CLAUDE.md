...
- **"Not sure what to make" concierge (2026-08-08)** — Create-page link + library empty-search rescue: three fixed questions (occasion/topic/time) then POST /api/games/suggest (Sonnet) returning only validated references (engine/suggest-validate.js: featured game ids, real recipes with legal params, bricks-only storyboards; everything else dropped server-side). Cards show real catalog data + one AI why-line; actions: Host / prefilled recipe form / seeded storyboard approval. Honest note when the ask exceeds the platform.
- **Customize asks first (2026-08-08)** — library Customize opens a "Make it yours" dialog: `POST /api/games/customize-questions` (Haiku) asks 2-3 activity-specific questions; answers ride the existing validated `/api/games/revise` flow (words only, structure preserved); skip and every failure path fall back to the plain copy
...
- **AI game generation is bricks-only (2026-08-07)** — the legacy whole-config generator (UI + `/api/games/generate` + `/api/games/generate-questions` + AIService.generateGame/generateQuestions/_autoPolishConfig) is REMOVED: if an activity can't be assembled from validated storyboard bricks, it can't be made (raw-config generation broke too many ways). Custom creation = recipe match or storyboard → compileStoryboard. GAME_GENERATOR_PROMPT + _fixGeneratedConfig survive (shared by Ask-AI revise flows).
...
- **No em dashes in user-facing text (2026-08-08)** — students read them as an AI tell. Full sweep done (configs, recipes, screens HTML, JS string literals; ~500 sites); use a comma, colon, or separate sentence instead. En dashes in numeric ranges (20–45 min) fine; the attributed Along bank stays verbatim; code comments/docs exempt. Enforced by `tests/style/no-em-dash.test.js` (acorn string-literal scanner + self-test); AI output covered by `STYLE_RULES` appended to every system prompt in `AIService._callClaude`.
...
- **Mic input (2026-08-07)** — `screens/shared/speech-input.js` (browser global `Speech`, side-effect-importable): "🎤 Speak instead" button on the player collect textarea + compact per-field mics on multi-field collects, via the browser's built-in Web Speech API (Chrome/Chromebooks). Feature-detected (no button on unsupported browsers; "Mic unavailable" when admin-blocked), dictation merges with typed text (never overwrites), respects maxLength, fires input events (counters react), stops on section change/submit. NO audio reaches the server — only final text through the normal submit path + content filter (recognition itself runs through the browser's speech service, i.e. Google on Chromebooks). Prototype player iframes carry `allow="microphone"`. The mic is a transparent 🎤 icon in the top-right corner INSIDE the text box (attachMic wraps the input in a `.mic-wrap`; flex parents handled). Teacher side: `Speech.autoAttach()` (designer + editor + library) puts the icon on EVERY text box, current and future-rendered, via sweep + MutationObserver — Simple view boxes, Builder rail, Ask AI, recipe params, dialogs, search — no per-form wiring (`.no-mic` ancestor class opts an area out). Mic styles live in shared/design.css (theme vars win, design tokens fall back).
...
- **991 tests passing** · **261 prompts** across 3 banks (recipes/prompt-banks/) (`npm test`)
...
- **AI activity creation** — Describe an activity in plain English: the from-description matcher fills a recipe, or the storyboard flow (`POST /api/games/storyboard`) proposes bricks the teacher approves before deterministic compilation. The legacy whole-config generator was removed 2026-08-07. Phase docs in AI prompts derive from `phase-schemas.js`.
