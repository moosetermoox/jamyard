...
## Surfaces & Routes
- `/` home (13a, 2026-09-06, copy by the job since 2026-09-10: headline "Get every student participating, in an activity made for your class.", ONE red action "Find one for today" + Paper door "Or say what you need and the AI builds it" on the left; the yard's board carousel with projector prints on the right (14.5s per slide, description shown in full); beneath, **eight boards, two per goal** (`HOME_PICKS` in the home script, shortest-first fill), each the carousel's board a size down, in rows named by the job (Get to know each other / Think it through together / Check what stuck / Play, and learn anyway), each wearing the yard's hover card (`screens/shared/hover-card.js`); the 1-2-3 under the boards, bold lines only; student join in the header strip and the join board; `screens/home/shots/` are load-bearing for the carousel AND the boards, rerun regen-carousel-shots.js after a projector redesign)
- `/library` — teacher front door: search, goal chips (Connect/Think/Review/Play; the time chips came out 2026-09-10, `minutes` on `/api/games` still feeds the planks), planks carry a `hook` line (recipe tagline or first sentence, `activityHook` in engine/home-glimpse.js), ▶ Host cards, ♥/recents, Customize dialog
