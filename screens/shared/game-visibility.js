...
//
// The site is public, but visitors only see the curated set: activities
// marked `featured` (built-in or owner-featured user creations), plus
// activities created on THIS device (ids remembered by my-games.js).
// Owner mode (owner-mode.js) shows everything. Used by the designer grid,
// the host picker, and prototype mode so the three lists can never drift
// apart.
//
  ...
  function visibleGames(games, opts) {
    ...
    return games.filter(function (game) {
      if ((game.source || 'built-in') === 'user') {
        return myIds.indexOf(game.id) !== -1 || !!game.featured;
      }
