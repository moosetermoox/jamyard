...
- draw: students draw on their devices, the teacher looks the drawings over first (built in, nothing drawn reaches the projector without that look), then the projector shows them ONE AT A TIME while each artist gets a word. text = what to draw. gallery = the line over the gallery (optional). timer = seconds to draw (optional, default 90). Use draw for any "draw a...", sketch, doodle, or picture idea. Drawings cannot be voted on, captioned by classmates, ranked, or scored (a ballot shows only text), so when the teacher wants a favorite ("vote for the scariest"), the drawing is the heart and the favorite is the class's show of hands during the gallery: BUILD the draw step and write the show of hands into the gallery line, never a vote step over drawings and never a cantBuild for a missing drawing vote.
