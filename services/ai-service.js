...
const PHASE_EXTRA_GUIDANCE = {
  ...
  collect:
    ...
    BLIND CHAINS (exquisite corpse): leave {{<phaseId>.assigned}} OUT of a rotating step's prompt and the student contributes without seeing what they received, the hand-off still happens in the data. End a chain with a return-to-author reveal: { "type": "reveal", "scope": "own", "chainFrom": ["starter", "round1", "round2"] } shows each student privately what became of the item THEY started (chainFrom lists the chain's steps in order). Add "chainDisplay": "template" plus "chainTemplate": "The {1} {2} {3}." to assemble one-word blind contributions into a sentence ({1} = the first step's word, {2} the next, and so on). A return-to-author reveal also stores every finished chain as {{<revealId>.responses}} (one row per starter: playerId, name, text), so a reveal-one gallery, a vote, or a rate step after it can read the chains; put the chains on the projector that way when the class should see or judge them. WHEN A TEACHER CHANGES A CHAIN (new links, phrases instead of words, a new theme): rewrite chainTemplate to fit the new links (a chain of phrases reads best as "{1} → {2} → {3}", never "The {1} {2}" over phrases), rewrite EVERY announce, prompt, and end message that describes the mechanic (how many hands, what each adds, "six words"), and rename the activity when its theme changed. Stale intro copy is the most common failure.
...
- When the request changes the theme, the topic, or how a step works, every message that DESCRIBES it changes too: the intro announce, the end message, counts like "six words" or "five classmates", a chain's chainTemplate, and the activity's name. Leaving the old intro over a new activity is a failure, not a small change.
- A step that shows results or has something to ask (discussionPrompt) gets NO timer: the teacher paces it.
...
- draw: students draw on their devices, the teacher looks the drawings over first (built in, nothing drawn reaches the projector without that look), then the projector shows them ONE AT A TIME while each artist gets a word. text = what to draw. gallery = the line over the gallery (optional). timer = seconds to draw (optional, default 90). Use draw for any "draw a...", sketch, doodle, or picture idea. When the teacher wants a favorite ("vote for the scariest"), put a vote step right after the draw step: the ballot shows the drawings as thumbnails, nobody votes for their own, and the winning drawing goes up under the crown. Drawings still cannot be captioned by classmates, ranked, or scored by points.
- summarize: the answers from the collect step before it are read and summed up, and the summary goes on the projector. text = how to sum them up ("group the answers into three themes, a short heading each, one quoted answer under each, no names"; "list the three most common ideas"; "write one paragraph of what the class thinks"). heading = the line the class reads above the summary (optional). REQUIRES a collect step before it. Use summarize for themes, clusters, "what the class is saying", the gist, most common ideas; the words students and the projector read never name the AI (say "the answers get summed up", never who does it).
