...
registerHandler('wager', {
  async onEnter(ctx) {
    ...
    for (const player of engine.players.list()) {
      if (wgEligibleIds.has(player.id)) {
        ...
        ctx.emitToPlayer(player.id, 'wager-start', {
          prompt: phase.prompt, options: wgOptions,
        ...
        ctx.emitToPlayer(player.id, 'waiting', { message: 'Waiting for others to place wagers...' });
      }
    }
  ...
  onReconnect(ctx, socket) {
    const wgState = ctx.room.wagerState;
    if (wgState) {
      const sc = ctx.resolveScreenControl();
