...
### START HERE next session (updated 2026-09-13 late: PRs #18 to #27 merged and live, 1963 tests)

**Later that night, the owner's next list, all merged (#25, #27 carrying #26).** Try it out: the arrow keys step through students from inside any frame. The make page: Closer shows its questions tier by tier in one fold (`talkQuestionsFor`); Doodle Bluff's "Who writes the phrases?" explains one answer at a time (`valueHelp` on recipe enums), picking "ai" writes 36 editable phrases onto the page (`setup.writes` on a text knob, `POST /api/games/phrase-list`, the compile flips to a teacher list), the premade list is 36; Group Work Day is recipe-born (`recipes/group-work-day.json`: students per group, how groups form, the jobs, how jobs are given, the tasks; a "Job: ..." line is that job's task, tagged at game time); a knobs panel that holds the words skips the AI fit questions (`state.contentKnobs`). OPEN from this: the group-work recipe (the older, jobless one) still exists beside group-work-day; Doodle Bluff's "ai" with an empty list still writes one per student at game time (by design, but the help line is long); nothing here has been seen in a real classroom.
