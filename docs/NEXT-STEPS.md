...
2. **Juice pack** (last item from the 2026-06 strategic review). Sounds, confetti,
   simple avatars, phase-transition flair. The games work; they don't yet *feel*
   like a party. `roomSpeak()` in host.js is the audio seed. Keep it config-driven
   (theme-level, not per-game hacks).
3. **AI cost guards.** No app-level rate limit or spend cap on AI endpoints; a
...
4. **Recipe compiler conditionals.** Hit 3× during Connection Pack (Closer's
...
5. **Remaining safety items** (from SAFETY-DESIGN.md): anonymous mode option,
   rate limiting / DoS limits, PII redaction.
6. **Editor odds and ends:**
...
7. **Accounts / owner model.** Game configs already persist in Neon Postgres;
   ...
   user-content line.
8. **Connection Pack voice modes** — v1.5 recorded clips, v2 WebRTC (staged in
   connection-pack-spec.md; deliberately NOT built yet).
9. **game-loader as a compiler** (architecture review item — see DEFERRED-IDEAS.md).
...
- Strategic review items 1–3: idea-first front door, robot playtest, Simple view ✅
