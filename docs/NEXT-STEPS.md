...
### START HERE next session (updated 2026-09-22: the new home is LIVE, PRs #62 and #63 MERGED; PR #64 open, 2390 tests)

**2026-09-22: the home the owner picked is live, and one PR waits.** The owner looked at both home rounds and said "I like this new version. Let's merge it": **#62** (identity headline, moment chips, `when` on the featured built-ins) merged first, then **#63** (the design handoff's 16h fold and the owner's round-two list) retargeted onto master and merged; CI green, Render deployed, merged master booted and served locally. What is live now: the headline "Get the whole class / in on it." beside **the picture** (`screens/shared/fold-picture.js` + `.css`, a drawn projector and one student's screen playing THREE scenes once on load, about fourteen seconds: Snowball's join and first answer, the pair's ONE shared box with the pen passing and "We agree, submit", then the drawing wall; a Replay button under it, never a mouse sweep; a fixed 660 by 330 canvas scaled with CSS zoom, stacked under the headline below 1100px, the finished still on a phone; the markup carries the finished state so no script = done), "What does your class need?" in spaced caps, the four planks with the ways on HOVER (the owner chose hover over always-on to compare; the day-old always-on version is in #62's history if they want it back), NO carousel (its painted parts moved into fold-picture.css), the 13a **how-it-works strip flat in one row above the yard** (step one says "Find one in the yard and make it yours", the 15b vocabulary test forbids "Pick an activity"), the yard head "The yard" with "Activities ready to be made to fit your class" beside it, the moment chip **"Only 5 minutes left"** (Exit Ticket, Live Poll, Solo Quiz; the owner disliked "before the bell"), the AI door as **"Have an idea? Make it real"** with one hover card ("When what your class needs isn't on this shelf yet. Say it in a sentence and it gets built while you watch."; a native title tooltip had made two popups), and **Guess Who: Rose, Bud, Thorn** hovering with the owner's line through `engine/when-lines.js` (a map by id for rows that live only in the database). Guards: `tests/screens/home-16h.test.js`, `tests/engine/when-lines-fallback.test.js`. **PR #64 OPEN (branch `when-setting`, owner: "Sounds good. Let's build it"):** "When is this for?" as a two-row box under Description in the designer's Settings panel, rendered and read back (blank drops the field), validated text-or-nothing on both sides, carried through both AI edit sites (`carryWhen`); guard `tests/screens/when-setting.test.js`. **Next:** (1) the owner merges #64 when they like it (auto-merge is OFF for the repo: `gh pr merge --auto` fails, merge by hand after CI); (2) once it is live, the owner opens Rose, Bud, Thorn in the designer on the live site, pastes the advisory line into the box, saves, and the row in `engine/when-lines.js` can go; (3) the owner's open questions from the day: whether "Activities" beside "The yard" should instead RENAME the head, whether the AI door copy should be cheekier, and whether fourteen seconds of picture is too long (the drawing scene is the cheapest cut, the shared box is the mechanic they asked to see); (4) the older items below (Chromebook websocket check, chaos suite on the starter five, Render off the free tier before the September 29 week, the pilot list) still stand. Gotchas from the day: in a column flex layout a `flex-basis: 660px` on the picture became its HEIGHT (a 300px void; `flex: 0 1 auto`); backticks inside a double-quoted `node -e` string are a command substitution in bash (write a script file); `gh pr merge` on a PR whose base just moved needs `gh pr update-branch` first, then CI, then the merge.

### Previous START HERE (2026-09-21: PRs #57, #58, #59 all MERGED and LIVE, 2360 tests)
