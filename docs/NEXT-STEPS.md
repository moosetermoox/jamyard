...
**2026-09-10: three tries on the yard and home (branch `yard-home-tries`, PR open).** Time chips out of the yard; the home shelf is eight boards in job-named rows; header and hero copy by the job it does (CHANGELOG 2026-09-10). Open: the row jobs could also subtitle the yard's four piles; Exit Ticket's board shows a join code, not a question (it is join-first), which reads differently from the other seven; the design canvas "Home, Eight Boards" still shows the boards-as-hero alternative if the carousel ever goes.
